...
parser.add_argument('--frame_margin', type=float, default=5,
...
parser.add_argument('--input_dir', type=str, default='',
                    help="directory with all input videos")
...
def runIt(frameRate, SAMPLE_RATE, SILENT_THRESHOLD, FRAME_SPREADAGE, NEW_SPEED, INPUT_FILE, FRAME_QUALITY):
    ...
    command = "ffmpeg -i "+'"'+INPUT_FILE+'"'+" -qscale:v " + \
    ...
    for chunk in chunks:
        audioChunk = audioData[int(
            chunk[0]*samplesPerFrame):int(chunk[1]*samplesPerFrame)]
        ...
        with WavReader(sFile) as reader:
            with WavWriter(eFile, reader.channels, reader.samplerate) as writer:
                tsm = phasevocoder(
                    reader.channels, speed=NEW_SPEED[int(chunk[2])])
    ...
    command = "ffmpeg -framerate "+str(frameRate)+" -i "+TEMP_FOLDER + \
        "/newFrame%06d.jpg -i "+TEMP_FOLDER+"/audioNew.wav -strict -2 "+'"'+OUTPUT_FILE+'"'
    subprocess.call(command, shell=True)

    shutil.rmtree(TEMP_FOLDER)


if INPUT_FILE == None and input_dir == "":
    print("you have to specify --input_file or --input_dir")
    exit

elif INPUT_FILE != None:
    runIt(frameRate, SAMPLE_RATE, SILENT_THRESHOLD, FRAME_SPREADAGE, NEW_SPEED, INPUT_FILE, FRAME_QUALITY)
    print("done ...")
    ...
    print("Some thing goes wrong with your input.")
    exit
